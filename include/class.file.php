...
class AttachmentFile extends VerySimpleModel {
    ...
    static function uploadBackdrop_sys(array $file, &$error) {
        if(extension_loaded('gd')) {
            $source_path = $file['tmp_name'];
            list($source_width, $source_height, $source_type) = getimagesize($source_path);
        ...
        return self::upload($file, 'S', false);
    }

    static function uploadBackdrop_landing(array $file, &$error) {
        if(extension_loaded('gd')) {
            $source_path = $file['tmp_name'];
            list($source_width, $source_height, $source_type) = getimagesize($source_path);
        ...
        return self::upload($file, 'H', false);
    }
    ...
    //changes 03/01/2023 get all backdrops sys
    static function allBackdrops_sys() {
        return static::objects()
            ->filter(array('ft' => 'S'))
            ->order_by('created');
    }

    static function allBackdrops_landing() {
        return static::objects()
            ->filter(array('ft' => 'H'))
            ->order_by('created');
    }
