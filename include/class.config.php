...
class OsticketConfig extends Config {
    ...
    function updatePagesSettings($vars, &$errors) {
        ...
        // changes 03/01/2023 add landing and system backdrops
        if($_FILES['backdrop_sys']) {
            $error = false;
            list($backdrop_sys) = AttachmentFile::format($_FILES['backdrop_sys']);
            if (!$backdrop_sys)
                ; // Pass
            elseif ($backdrop_sys['error'])
                $errors['backdrop_sys'] = $backdrop_sys['error'];
            elseif (!AttachmentFile::uploadBackdrop_sys($backdrop_sys, $error))
                $errors['backdrop_sys'] = sprintf(__('Unable to upload backdrop image: %s'), $error);
        }
        if($_FILES['backdrop_landing']) {
            $error = false;
            list($backdrop_landing) = AttachmentFile::format($_FILES['backdrop_landing']);
            if (!$backdrop_landing)
                ; // Pass
            elseif ($backdrop_landing['error'])
                $errors['backdrop_landing'] = $backdrop_landing['error'];
            elseif (!AttachmentFile::uploadBackdrop_landing($backdrop_landing, $error))
                $errors['backdrop_landing'] = sprintf(__('Unable to upload backdrop image: %s'), $error);
        }
        $company = $ost->company;
        ...
        if (isset($vars['delete-backdrop']))
            foreach ($vars['delete-backdrop'] as $id)
                if (($vars['selected-backdrop'] != $id)
                    ...
                    $f->delete();
        //changes 03/01/2023
        if (isset($vars['delete-backdrop_sys']))
            foreach($vars['delete-backdrop_sys'] as $id)
                if (($vars['selected-backdrop_sys'] != $id)
                        && ($f = AttachmentFile::lookup((int) $id)))
                    $f->delete();

        if (isset($vars['delete-backdrop_landing']))
        foreach($vars['delete-backdrop_landing'] as $id)
            if (($vars['selected-backdrop_landing'] != $id)
                    && ($f = AttachmentFile::lookup((int) $id)))
                $f->delete();
        ...
        return $this->updateAll(array(
            ...
            'admin_backdrop_sys_id' => (
                (is_numeric($vars['selected-backdrop_sys']) && $vars['selected-backdrop_sys'])
                ? $vars['selected-backdrop_sys'] : false),
            'agent_backdrop_sys_id' => (
                (is_numeric($vars['selected-backdrop_sys_agent']) && $vars['selected-backdrop_sys_agent'])
                ? $vars['selected-backdrop_sys_agent'] : false),
            'staff_backdrop_landing_id' => (
                (is_numeric($vars['selected-backdrop_landing']) && $vars['selected-backdrop_landing'])
                ? $vars['selected-backdrop_landing'] : false),
           ));
    }
